...
class Database:
    ...
    def create_tables(self):
        ...
        try:
            ...
            # Create relevant experience projects table
            cursor.execute('''
            CREATE TABLE IF NOT EXISTS relevant_experience_projects (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                freelancer_id INTEGER,
    ...
    def add_freelancer_profile(self, name, email, hourly_rate, skills, experience_years, bio=None, portfolio_url=None, github_url=None, linkedin_url=None, relevant_experience=None, timezone=None):
        cursor = self._get_cursor()
        try:
            cursor.execute('''
            INSERT INTO freelancer_profiles (name, email, hourly_rate, skills, experience_years, bio, portfolio_url, github_url, linkedin_url, relevant_experience, timezone)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (name, email, hourly_rate, json.dumps(skills), experience_years, bio, portfolio_url, github_url, linkedin_url, relevant_experience if relevant_experience else None, timezone))
            self.conn.commit()
    ...
    def update_freelancer_profile(self, freelancer_id, **kwargs):
        ...
        try:
            ...
            for key, value in kwargs.items():
                if key in ['name', 'email', 'hourly_rate', 'experience_years', 'bio', 'portfolio_url', 'github_url', 'linkedin_url', 'relevant_experience', 'timezone', 'availability_status']:
        ...
        try:
            ...
            if profile:
                return {
                    ...
                    'skills': self._parse_skills(profile['skills']),
                    'experience_years': profile['experience_years'],
                    'bio': profile['bio'],
                    'portfolio_url': profile['portfolio_url'],
                    'github_url': profile['github_url'] if 'github_url' in profile.keys() else None,
                    'linkedin_url': profile['linkedin_url'] if 'linkedin_url' in profile.keys() else None,
                    'relevant_experience': profile['relevant_experience'] if 'relevant_experience' in profile.keys() else None,
    ...
    def add_scraped_job(self, job_title, job_url, job_description, required_skills, client_name,
        ...
        try:
            cursor.execute('''
            INSERT OR IGNORE INTO scraped_jobs (job_title, job_url, job_description, required_skills, client_name,
                                    client_rating, client_total_jobs, client_total_hires, client_avg_review,
                                    budget_range, avg_pay_rate, project_duration, job_category, posted_date)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    def clear_scraped_jobs(self):
        """Clear all scraped jobs from the database"""
        cursor = self._get_cursor()
        try:
            ...
            print(f"Error clearing scraped jobs: {e}")
            self.conn.rollback()
            return False
        finally:
            cursor.close()
    ...
    def get_all_freelancer_profiles(self):
        ...
        try:
            ...
            import traceback
            print(f"Error getting all freelancer profiles: {e} ({type(e)})")
            traceback.print_exc()
            return []
        finally:
            cursor.close()
    ...
    def add_relevant_experience_project(self, freelancer_id, project_title, project_description, 
                                      project_url=None, company_name=None, project_type=None,
                                      technologies_used=None, key_achievements=None, 
                                      project_duration=None, completion_date=None):
        cursor = self._get_cursor()
        try:
            cursor.execute('''
            INSERT INTO relevant_experience_projects (freelancer_id, project_title, project_description,
                                                    project_url, company_name, project_type,
                                                    technologies_used, key_achievements,
                                                    project_duration, completion_date)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (freelancer_id, project_title, project_description, project_url, company_name,
                 project_type, json.dumps(technologies_used) if technologies_used else None,
                 key_achievements, project_duration, completion_date))
            self.conn.commit()
            return cursor.lastrowid
        except sqlite3.Error as e:
            print(f"Error adding relevant experience project: {e}")
            self.conn.rollback()
            return None
        finally:
            cursor.close()

    def get_relevant_experience_projects(self, freelancer_id):
        cursor = self._get_cursor()
        try:
            cursor.execute('SELECT * FROM relevant_experience_projects WHERE freelancer_id = ? ORDER BY completion_date DESC', (freelancer_id,))
            projects = cursor.fetchall()
            return [{
                'id': project['id'],
                ...
                'technologies_used': self._parse_json_field(project['technologies_used']),
            ...
            print(f"Error getting relevant experience projects: {e}")
            return []
        finally:
            cursor.close()

    def update_relevant_experience_project(self, project_id, **kwargs):
        cursor = self._get_cursor()
        try:
            update_fields = []
            values = []
            ...
            for key, value in kwargs.items():
                if key in ['project_title', 'project_description', 'project_url', 'company_name', 
                          'project_type', 'key_achievements', 'project_duration', 'completion_date']:
                    update_fields.append(f"{key} = ?")
                    values.append(value)
                elif key == 'technologies_used':
                    update_fields.append("technologies_used = ?")
                    values.append(json.dumps(value))
            
            if update_fields:
                values.append(project_id)
                query = f"UPDATE relevant_experience_projects SET {', '.join(update_fields)} WHERE id = ?"
                cursor.execute(query, values)
                self.conn.commit()
                return cursor.rowcount > 0
            return False
        except sqlite3.Error as e:
            print(f"Error updating relevant experience project: {e}")
            self.conn.rollback()
            return False
        finally:
            cursor.close()

    def delete_relevant_experience_project(self, project_id):
        cursor = self._get_cursor()
        try:
            cursor.execute('DELETE FROM relevant_experience_projects WHERE id = ?', (project_id,))
            self.conn.commit()
            return cursor.rowcount > 0
        except sqlite3.Error as e:
            print(f"Error deleting relevant experience project: {e}")
            self.conn.rollback()
            return False
        finally:
            cursor.close() 